...
void SkiaWindowX11::onPaint(const gfx::Rect& rc)
{
  ...
  if (scale == 1) {
    ...
    const SkImageInfo info = SkImageInfo::Make(
      rc.w, rc.h, bitmap.info().colorType(), bitmap.info().alphaType());
    ...
    if (scaled.installPixels(info, (void*)m_buffer.data(), rowBytes)) {
      ...
      SkRect srcRect = SkRect::Make(SkIRect::MakeXYWH(
        rc.x / scale, rc.y / scale, rc.w / scale, rc.h / scale));
      SkRect dstRect = SkRect::Make(SkIRect::MakeXYWH(0, 0, rc.w, rc.h));
      canvas.drawImageRect(
        SkImage::MakeFromRaster(bitmap.pixmap(), nullptr, nullptr),
        srcRect,
        dstRect,
        SkSamplingOptions(),
        &paint,
