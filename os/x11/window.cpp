  ...
  , m_transparent(spec.transparent())
{
  ...
  if (!_NET_FRAME_EXTENTS) {
    ...
    _NET_REQUEST_FRAME_EXTENTS =
      XInternAtom(m_display, "_NET_REQUEST_FRAME_EXTENTS", False);
  }
  if (!_NET_WM_STATE) {
  ...
  if (!_NET_WM_ALLOWED_ACTIONS)
    _NET_WM_ALLOWED_ACTIONS =
      XInternAtom(m_display, "_NET_WM_ALLOWED_ACTIONS", False);
  ...
  if (m_transparent) {
    Status s =
      XMatchVisualInfo(m_display, DefaultScreen(m_display), 32, TrueColor, &vi);
    if (s == 0) {
  ...
  swa.event_mask =
    (StructureNotifyMask | ExposureMask | PropertyChangeMask | EnterWindowMask |
  ...
  if (spec.floating()) {
    ...
    if (_NET_WM_WINDOW_TYPE && _NET_WM_WINDOW_TYPE_UTILITY &&
  ...
  // behavior at all).
  {
    ...
    static_assert(sizeof(hints) / sizeof(long) == 5,
                  "Invalid MotifHints struct");
  }
  ...
  if (spec.floating() && spec.parent()) {
    ...
    XSetTransientForHint(
      m_display, m_window, static_cast<WindowX11*>(spec.parent())->m_window);
  }
...
{
  const Atom _NET_ACTIVE_WINDOW =
    XInternAtom(m_display, "_NET_ACTIVE_WINDOW", False);
...
void WindowX11::maximize()
{
  ...
  event.xclient.data.l[0] =
    (isMaximized() ? _NET_WM_STATE_REMOVE : _NET_WM_STATE_ADD);
...
void WindowX11::setFullscreen(bool state)
{
  ...
  event.xclient.data.l[1] =
    _NET_WM_STATE_FULLSCREEN;   // First property to alter
...
gfx::Rect WindowX11::contentRect() const
{
  ...
  XGetGeometry(
    m_display, m_window, &root, &x, &y, &width, &height, &border, &depth);
...
bool WindowX11::setCursor(NativeCursor nativeCursor)
{
  const CursorRef cursor =
    ((SystemX11*)os::instance())->getNativeCursor(nativeCursor);
  if (cursor)
...
void WindowX11::performWindowAction(const WindowAction action, const Event* ev)
{
  ...
  if (ev) {
    ...
    ::Window root, child;
    if (!XQueryPointer(
          m_display, m_window, &root, &child, &rootx, &rooty, &x, &y, &mask)) {
      x = 0;
      y = 0;
    }
  }
  ...
  switch (action) {
    case WindowAction::Cancel:
      direction = _NET_WM_MOVERESIZE_CANCEL;
      break;
...
void WindowX11::setAllowedActions()
{
  ...
  // property.
  auto set_allowed_action = [&allowed, this](const bool expected,
                                             const char* atomName) {
...
bool WindowX11::requestX11FrameExtents()
{
  ...
  while (wait > 0) {
    if (XCheckIfEvent(
          m_display, &event2, isFrameExtentsEvent, (XPointer)m_window)) {
  ...
  switch (event.type) {
    ...
    case KeyRelease: {
      ...
      if (m_xic) {
        ...
        const size_t len = Xutf8LookupString(
          m_xic, &event.xkey, buf.data(), buf.size(), nullptr, nullptr);
        if (len < buf.size())
          buf[len] = 0;
    ...
    case ButtonRelease: {
      ...
      if (is_mouse_wheel_button(event.xbutton.button)) {
        ...
        if (event.type == ButtonPress) {
          if (m_doubleClickButton == button &&
              base::current_tick() - m_doubleClickTick <
                LAF_X11_DOUBLE_CLICK_TIMEOUT) {
      ...
      ev.setPosition(
        gfx::Point(event.xbutton.x / m_scale, event.xbutton.y / m_scale));
    ...
    case LeaveNotify:
      ...
      if (event.xcrossing.mode == NotifyNormal) {
        ...
        ev.setPosition(
          gfx::Point(event.xcrossing.x / m_scale, event.xcrossing.y / m_scale));
        queueEvent(ev);
      }
      break;
