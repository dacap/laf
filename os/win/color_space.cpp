...
os::ColorSpaceRef get_colorspace_from_icc_file(const std::string& iccFilename)
{
  ...
  auto osCS =
    os::instance()->makeColorSpace(gfx::ColorSpace::MakeICC(std::move(buf)));
  if (osCS) {
    osCS->gfxColorSpace()->setName("Display Profile: " +
...
{
  EnumDisplayMonitors(
    nullptr, nullptr, list_display_colorspaces_enumproc, (LPARAM)&list);
}
