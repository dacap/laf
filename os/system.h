...
public:
  ...
  virtual Ref<Surface> makeSurface(
    int width, int height, const os::ColorSpaceRef& colorSpace = nullptr) = 0;
#if CLIP_ENABLE_IMAGE
  virtual Ref<Surface> makeSurface(const clip::image& image) = 0;
#endif
  virtual Ref<Surface> makeRgbaSurface(
    int width, int height, const os::ColorSpaceRef& colorSpace = nullptr) = 0;
  ...
  virtual Ref<Font> loadSpriteSheetFont(const char* filename,
                                        int scale = 1) = 0;
  ...
  virtual gfx::Color getColorFromScreen(
    const gfx::Point& screenPosition) const = 0;
  ...
  virtual Ref<ColorSpaceConversion> convertBetweenColorSpace(
    const os::ColorSpaceRef& src, const os::ColorSpaceRef& dst) = 0;
