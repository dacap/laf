...
public:
  constexpr Matrix() { }
  constexpr Matrix(const SkMatrix& skMatrix)
    : m_skMatrix(skMatrix)
  {
  }
  ...
  static Matrix MakeAll(float scaleX,
                        float skewX,
                        float transX,
                        float skewY,
                        float scaleY,
                        ...
                        float pers2)
  {
    return Matrix(SkMatrix::MakeAll(
  ...
  void setScale(float sx, float sy) { m_skMatrix.setScale(sx, sy); }
  ...
  void setRotate(float degrees) { m_skMatrix.setRotate(degrees); }
  ...
  RectF mapRect(const RectF& src) const
  {
    ...
    m_skMatrix.mapRect(
      &dst,
      SkRect::MakeXYWH(
        SkScalar(src.x), SkScalar(src.y), SkScalar(src.w), SkScalar(src.h)));
