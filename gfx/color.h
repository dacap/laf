...
inline constexpr Color rgba(ColorComponent r,
                            ColorComponent g,
                            ColorComponent b,
                            ColorComponent a = 255)
{
  return Color((r << ColorRShift) | (g << ColorGShift) | (b << ColorBShift) |
               (a << ColorAShift));
}

inline constexpr ColorComponent getr(Color c)
{
  return (c >> ColorRShift) & 0xff;
}
inline constexpr ColorComponent getg(Color c)
{
  return (c >> ColorGShift) & 0xff;
}
inline constexpr ColorComponent getb(Color c)
{
  return (c >> ColorBShift) & 0xff;
}
inline constexpr ColorComponent geta(Color c)
{
...
inline constexpr bool is_transparent(Color c)
{
  return geta(c) == 0;
}
