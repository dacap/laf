...
{
  switch (a) {
    case hkey::access::read:
      return KEY_READ;
    case hkey::access::write:
...
hkey hkey::open(const std::string& subkey, const access a)
{
  ...
  LONG result =
    RegOpenKeyExW(m_hkey, from_utf8(subkey).c_str(), 0, to_regsam(a), &openKey);
...
bool hkey::exists(const std::string& name)
{
  ...
  LONG result =
    RegQueryValueExW(m_hkey, wname.c_str(), nullptr, nullptr, nullptr, nullptr);
...
std::string hkey::string(const std::string& name)
{
  ...
  LONG result = RegQueryValueExW(
    m_hkey, wname.c_str(), nullptr, (LPDWORD)&type, nullptr, (LPDWORD)&reqSize);
  ...
  result = RegQueryValueExW(m_hkey,
                            wname.c_str(),
                            nullptr,
                            nullptr,
                            (LPBYTE)&buf[0],
