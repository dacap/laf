...
static void throw_cannot_open_exception(const string& filename,
                                        const string& mode)
{
...
#else
  return freopen(
    (!filename.empty() ? filename.c_str() : nullptr), mode.c_str(), stream);
#endif
}
...
#else
  fd = open(
    filename.c_str(), flags, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
#endif
